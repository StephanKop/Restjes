...
function SocialIcon({ children, href, label }: { children: React.ReactNode; href: string; label: string }) {
  return (
    <a
      ...
      aria-label={label}
      className="flex h-10 w-10 items-center justify-center rounded-full bg-warm-100 text-warm-500 transition-colors hover:bg-brand-100 hover:text-brand-600"
    >
      {children}
    </a>
  )
            ...
            <ul className="space-y-2.5">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-warm-500 transition-colors hover:text-brand-600"
                  >
                    {link.label}
                  </Link>
                </li>
